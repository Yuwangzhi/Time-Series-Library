...
def main():
    print("=" * 72)
    print("            AUTO-RUN ALL LOAD_DATA MODEL SCRIPTS (60-MIN)")
    print("=" * 72)
    print("This script will run all available 60-minute granularity load_data model experiments in sequence.")
    print("Each model includes 4 prediction horizons: 96->96, 96->192, 96->336, 96->720")
    print("Data granularity: 60 minutes (1 hour)")
    print("Time interpretations:")
    print("- 96 time steps = 96 hours (96 × 60 min = 4 days)")
    print("- 192 time steps = 192 hours (192 × 60 min = 8 days)")
    print("- 336 time steps = 336 hours (336 × 60 min = 14 days)")
    print("- 720 time steps = 720 hours (720 × 60 min = 30 days)")
    ...
    print(f"🚀 Starting all 60-minute granularity model experiments at: {start_time.strftime('%Y-%m-%d %H:%M:%S')}")
    print()
    ...
    overall_log = os.path.join(results_dir, f"load_data_60min_all_models_log_{start_time.strftime('%Y%m%d_%H%M%S')}.txt")
    print(f"Overall experiment log: {overall_log}")
    print()
    
    # Initialize overall log
    with open(overall_log, 'w') as f:
        f.write("=== LOAD_DATA 60-MINUTE ALL MODELS EXPERIMENT LOG ===\n")
        f.write(f"Start Time: {start_time.strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"Data Granularity: 60 minutes (1 hour)\n")
    ...
    for i, script_name in enumerate(model_scripts, 1):
        ...
        # Extract model name
        model_name = script_name.replace('_load_data_60min.sh', '')
        ...
        with open(overall_log, 'a') as f:
            f.write(f"[{i}/{total_models}] Starting {model_name} (60-min) experiments...\n")
        ...
        if script_path.exists():
            ...
            try:
                ...
                if result.returncode == 0:
                    print(f"✅ {model_name} (60-min) completed successfully at: {model_end_time.strftime('%Y-%m-%d %H:%M:%S')}")
                    print(f"Duration: {duration}")
                    successful_models.append(model_name)
                    
                    with open(overall_log, 'a') as f:
                        f.write(f"[{i}/{total_models}] ✅ {model_name} (60-min) completed successfully at: {model_end_time.strftime('%Y-%m-%d %H:%M:%S')}\n")
                        f.write(f"Duration: {duration}\n")
                else:
                    print(f"❌ {model_name} (60-min) failed at: {model_end_time.strftime('%Y-%m-%d %H:%M:%S')}")
                    print(f"Duration: {duration}")
                    failed_models.append(model_name)
                    
                    with open(overall_log, 'a') as f:
                        f.write(f"[{i}/{total_models}] ❌ {model_name} (60-min) failed at: {model_end_time.strftime('%Y-%m-%d %H:%M:%S')}\n")
                        f.write(f"Duration: {duration}\n")
                        
            except Exception as e:
                model_end_time = datetime.datetime.now()
                print(f"❌ {model_name} (60-min) error: {str(e)}")
                failed_models.append(model_name)
                
                with open(overall_log, 'a') as f:
                    f.write(f"[{i}/{total_models}] ❌ {model_name} (60-min) error: {str(e)}\n")
        else:
    ...
    print("=" * 72)
    print("🎉 ALL 60-MINUTE GRANULARITY MODEL EXPERIMENTS COMPLETED!")
    print("=" * 72)
    print(f"Start Time: {start_time.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"End Time: {end_time.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"Total Duration: {total_duration}")
    print(f"Total Models Processed: {total_models}")
    print(f"Data Granularity: 60 minutes (1 hour)")
    ...
    print("- Individual model logs: ./results/long_term_forecast_load_data_60min_*/experiment_log.txt")
    print(f"- Overall experiment log: {overall_log}")
    print("- Model checkpoints: ./checkpoints/long_term_forecast_load_data_60min_*/")
    print()
    ...
    print("- 720 time steps = 720 hours (720 × 60 min = 30 days)")
    print()
    print(f"You can now analyze the results from all {len(successful_models)} successful 60-minute granularity models!")
    print("=" * 72)
