@echo off
REM Auto-run all load_data model scripts (Windows batch version - 15-minute granularity)
REM This script will automatically execute all 15-minute granularity model scripts in sequence
...
echo Starting all 15-minute granularity model experiments at: %date% %time%
echo.
...
set "OVERALL_LOG=..\..\..\results\load_data_15min_all_models_log_%date:~0,4%%date:~5,2%%date:~8,2%_%time:~0,2%%time:~3,2%%time:~6,2%.txt"
...
call :run_model "Autoformer_load_data_15min.sh" "Autoformer"
call :run_model "Crossformer_load_data_15min.sh" "Crossformer"
call :run_model "DLinear_load_data_15min.sh" "DLinear"
call :run_model "FEDformer_load_data_15min.sh" "FEDformer"
call :run_model "Informer_load_data_15min.sh" "Informer"
call :run_model "iTransformer_load_data_15min.sh" "iTransformer"
call :run_model "MICN_load_data_15min.sh" "MICN"
call :run_model "Nonstationary_Transformer_load_data_15min.sh" "Nonstationary_Transformer"
call :run_model "PatchTST_load_data_15min.sh" "PatchTST"
call :run_model "SegRNN_load_data_15min.sh" "SegRNN"
call :run_model "TimeMixer_load_data_15min.sh" "TimeMixer"
call :run_model "TimesNet_load_data_15min.sh" "TimesNet"
call :run_model "Transformer_load_data_15min.sh" "Transformer"
call :run_model "TSMixer_load_data_15min.sh" "TSMixer"
...
echo ========================================================================
echo 🔄 Running Model %MODEL_NUM%/%TOTAL_MODELS%: %MODEL_NAME% (15-min)
echo ========================================================================

echo [%MODEL_NUM%/%TOTAL_MODELS%] Starting %MODEL_NAME% (15-min) experiments... >> "%OVERALL_LOG%"
...
if exist "%SCRIPT_NAME%" (
    ...
    if %errorlevel% equ 0 (
        echo ✅ %MODEL_NAME% (15-min) completed successfully at: %date% %time%
        echo [%MODEL_NUM%/%TOTAL_MODELS%] ✅ %MODEL_NAME% (15-min) completed successfully at: %date% %time% >> "%OVERALL_LOG%"
    ) else (
        echo ❌ %MODEL_NAME% (15-min) failed at: %date% %time%
        echo [%MODEL_NUM%/%TOTAL_MODELS%] ❌ %MODEL_NAME% (15-min) failed at: %date% %time% >> "%OVERALL_LOG%"
    )
) else (
